...
public class Console {
    ...
    public static final String LOG_PATTERN = "%1$tH:%1$tM:%1$tS - [%2$-5s][%3$-8s]: %4$s %n";

    static {
        Identifier id = null;

        if (EnhancedRobotBase.RUNNING_INSTANCE != null)
            if (EnhancedRobotBase.RUNNING_INSTANCE.robotId != null)
                id = EnhancedRobotBase.RUNNING_INSTANCE.robotId;

        if (id == null) id = Identifier.newBasicID("FRC", 0);

        logger = Logger.getLogger(id.teamName() + "-" + id.teamNumberAsString());
        Thread.currentThread().setName("FRC-" + id.teamNumber());
